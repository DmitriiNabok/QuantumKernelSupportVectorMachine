import numpy as np
from qiskit.circuit import QuantumRegister, QuantumCircuit, ParameterVector, Parameter
...
def ZZ_FeatureMap(n_features, n_qubits, n_layers, data_map_func=None):
    ...
    for r in range(n_layers):
        ...
        for i in range(n_qubits-1):
            v = data_map_func(alpha*x[j%n_features], alpha*x[(j+1)%n_features]); j += 1
...
def SingleQubitWall(n_qubits, rotations=None, random_state=None):
    ...
    for i in range(n_qubits):
        for gate in [XGate(), YGate(), ZGate()]:
            qc.append(gate.power(rotations[j]), [i]); j += 1
