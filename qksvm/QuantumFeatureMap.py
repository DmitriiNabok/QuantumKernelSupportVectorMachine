...
class QuantumFeatureMap(QuantumCircuit):
    ...
    def setup_parameters(self, gates: Sequence[str]) -> None:
        ...
        if not any(t_list):
            print('\nWarning:')
            print('\tEncoding circuit is not specified. Is it what you are aiming at?\n')
    ...
    def build_circuit(self, 
        ...
        #-------------------------------------------------------------
        # Auxiliary exception for maintenting the repeating encoding
        # used for building a circuit
        #-------------------------------------------------------------
        ...
        try:
            ...
            if gate in ['crx', 'cry', 'crz', 'rxx', 'ryy', 'rzz', 'rzx', 'rzx']:
                ...
                for i in range(self.num_qubits):
                    if not self.repeat: check(j)
                    _gate(params[j%num_params], self.qubits[i]); j += 1
