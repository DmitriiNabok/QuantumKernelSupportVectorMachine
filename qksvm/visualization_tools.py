...
def plot_dataset(X, y, ax, axes, marker='o', size=50, alpha=1.0):
    """Simple routine to visualize a 2D dataset"""
    cm_bright = ListedColormap(['#FF0000', '#0000FF'])
    ax.scatter(X[:, 0], X[:, 1], c=y, cmap=cm_bright, edgecolors='k', marker=marker, s=50, alpha=alpha)
    ax.axis(axes)
...
def plot_predictions(clf, ax, axes, N=100):
    ...
    ax.contourf(x0, x1, y_pred, cmap=plt.cm.RdBu, alpha=0.9)

    
def plot_decisions(clf, ax, axes, N=100):
    ...
    ax.contourf(x0, x1, y_decision, cmap=plt.cm.RdBu, alpha=0.9)